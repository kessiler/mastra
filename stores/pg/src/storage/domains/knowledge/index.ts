...
export class KnowledgePG extends KnowledgeStorage {
  ...
  async createProposal(input: CreateKnowledgeProposalInput): Promise<KnowledgeProposal> {
    return this.#transaction(async tx => {
      ...
      await tx.execute({
        sql: `INSERT INTO "${TABLE_KNOWLEDGE_PROPOSALS}" (id,"targetType","targetId","expectedVersion",action,changes,reason,"proposerContextScopeId",status,"createdAt") VALUES (?,?,?,?,?,CAST(? AS jsonb),?,?,?,?)`,
        args: [
          id,
          primaryTarget.type,
          primaryTarget.id,
  ...
  async reviewProposal(input: ReviewKnowledgeProposalInput): Promise<KnowledgeProposal> {
    return this.#transaction(async tx => {
      await this.#assertExpectedAccessEpoch(tx, input.expectedAccessEpoch);
      const existing = await tx.execute({
        sql: `SELECT * FROM "${TABLE_KNOWLEDGE_PROPOSALS}" WHERE id=? FOR UPDATE`,
        args: [input.id],
      });
      ...
      const updated = await tx.execute({
        ...
        args: [input.status, input.reviewerContextScopeId, input.reviewReason ?? null, reviewedAt, input.id],
      });
  ...
  async claimSemanticOutbox(input: ClaimKnowledgeSemanticOutboxInput): Promise<KnowledgeSemanticOutboxEntry[]> {
    ...
    return this.#transaction(async tx => {
      ...
      const visibleCandidates: KnowledgeSemanticOutboxEntry[] = selected.rows.map(parseOutbox);
  ...
  async #updateNode(executor: Executor, input: UpdateKnowledgeNodeInput): Promise<KnowledgeNode> {
    ...
    await this.#lockSiblingName(executor, updated.name, scopeIds);
  ...
  async #markProposalConflicted(
    ...
    const updated = await tx.execute({
      ...
      args: [reviewerContextScopeId, reviewReason, reviewedAt, proposal.id],
    });
    if (updated.rowsAffected !== 1) throw new KnowledgeConflictError('Knowledge proposal was already reviewed');
  ...
  async #isProposalVisible(
    ...
    for (const target of proposal.targets) {
      ...
      if (target.type === 'node') {
        ...
        currentScopeIds = node.isScope ? [node.id] : await this.#getNodeScopeIds(executor, node.id);
